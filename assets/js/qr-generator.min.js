// NOTE: This file is intentionally kept functionally in sync with assets/js/qr-generator.js.
// It may not be minified in this environment.
...
document.addEventListener('DOMContentLoaded', function() {
	...
	async function addLogoToQR(settingsOverride) {
		...
		if (img) {
			try {
				...
				// Ignore if canvas becomes tainted (shouldn't happen with local uploads)
			}
		}
	}
	...
	window.reloadQR = function(index) {
		...
		document.getElementById('qr-error-level').value = item.settings.correctLevel;
		if (item.style && document.getElementById('qr-pattern')) document.getElementById('qr-pattern').value = item.style.dotsType || 'square';
		if (item.style && document.getElementById('qr-corner-square')) document.getElementById('qr-corner-square').value = item.style.cornerSquareType || 'square';
		if (item.style && document.getElementById('qr-corner-dot')) document.getElementById('qr-corner-dot').value = item.style.cornerDotType || 'dot';
		updateSizeValue();
