// NOTE: This file is intentionally kept functionally in sync with assets/js/password-generator.js.
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function generatePassword() {
		...
		if (options.noDuplicate && charset.length < length) {
			showAlert(
				tr('password.error_not_enough_chars', `Không đủ ký tự duy nhất (${charset.length}) cho độ dài mật khẩu (${length})!`),
				'warning'
			);
			return;
		}
	...
	function copyPassword() {
		...
		navigator.clipboard.writeText(currentPassword).then(() => {
			showAlert(tr('password.copied', 'Đã sao chép mật khẩu!'), 'success');

			// Visual feedback
			copyBtn.innerHTML = '<i class="fas fa-check"></i> ' + tr('password.copied_short', 'Đã sao chép');
			setTimeout(() => {
				copyBtn.innerHTML = '<i class="fas fa-copy"></i> ' + tr('password.copy', 'Sao chép');
			}, 2000);
			...
			showAlert(tr('password.copy_error', 'Không thể sao chép!'), 'danger');
		});
	}
	...
	function updateHistoryDisplay() {
		...
		historyContainer.innerHTML = passwordHistory.map((item) => `
	...
	function clearHistory() {
		...
		if (confirm(tr('password.confirm_clear', 'Xóa toàn bộ lịch sử?'))) {
			...
			saveToLocalStorage('passwordHistory', passwordHistory);
			updateHistoryDisplay();
			showAlert(tr('password.history_cleared', 'Đã xóa lịch sử!'), 'info');
		}
	}
